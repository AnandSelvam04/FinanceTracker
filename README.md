...
- SMS import: scans the last 90 days of the SMS inbox for bank transaction alerts, parses out amount, direction, merchant, and date, and routes each one to the account whose last-4 digits the message names. Money moving between your own accounts (a card bill paid from a bank, an NEFT between banks) is detected from the from/to pair and recorded as a transfer rather than as spending, and the two alerts one movement sends are collapsed into a single row. Candidates that match a transaction you already entered by hand are flagged and left unchecked. Everything lands in a review queue — nothing posts to the ledger without confirmation. Imported and dismissed messages are remembered, so a rescan never offers the same one twice.
