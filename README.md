...
## Features
- Transactions: record expenses and income with category/date/amount; transfers between accounts (including cross-currency transfers with a destination-side amount); search, type filters, and month/year rollups in the list view. The list groups rows under Today / Yesterday / date headers with each day's spend, and leads with the spent and income totals for whatever the current filters show.
